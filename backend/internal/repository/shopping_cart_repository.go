...
func (r *ShoppingCartRepository) DeleteItem(id, shoppingCartID int) error {
	return r.db.Where("id = ? AND shopping_cart_id = ?", id, shoppingCartID).Delete(&model.ShoppingCartItem{}).Error
}
...
func (r *ShoppingCartRepository) ClearCart(shoppingCartID int) error {
	return r.db.Where("shopping_cart_id = ? AND want_to_buy = ?", shoppingCartID, true).Delete(&model.ShoppingCartItem{}).Error
}
